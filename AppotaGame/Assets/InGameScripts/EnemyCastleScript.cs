    ...
    private void CheckNeedToMoveFlag(Collider other)
    {
        ...
        if (other.gameObject.GetComponent<UnitStatePattern>().CarryingFlag() && distance < 4)
        {
            ...
            for (int j = 1; j < placeHolder.childCount + 1; j++)
            {
                ...
                foreach (GameObject flag in GameAI.getGameAISingelton.GetBlueFlagList())
                {
            ...
            GameAI.getGameAISingelton.RemoveFriendlyFlag(toBeDesroyed);
            Destroy(toBeDesroyed);
            other.gameObject.GetComponent<UnitStatePattern>().Kill(); // kill the warrior who returned the flag
        }
    } 
}
